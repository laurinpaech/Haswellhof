...
FastHessian::FastHessian(struct integral_image*img, std::vector<Ipoint> &ipts,
                         const int octaves, const int intervals, const int init_sample,
                         const float thresh)
                         : ipts(ipts), i_width(0), i_height(0)
{
...
void FastHessian::setIntImage(struct integral_image*img)
{
...
void FastHessian::buildResponseMap()
{
  ...
  // TESTING
  // int num_rl = 6;
  // ResponseLayer* rl = responseMap[num_rl];
  //
  // for (int ind = 0; ind < 1000; ind++) {
  //     float responses = rl->responses[ind];
  //     float laplacian = rl->laplacian[ind];
  //     printf("response %i: %f\nlaplacian %i: %f\n\n", ind, responses, ind, laplacian);
  // }
  // TESTING
}
...
void FastHessian::buildResponseLayer(ResponseLayer *rl)
{
  ...
  for(int r, c, ar = 0, index = 0; ar < rl->height; ++ar)
  {
    for(int ac = 0; ac < rl->width; ++ac, index++)
    {
      ...
      // if (index == 997) {
      //   printf("Dxx: %f, Dyy: %f, Dxy: %f\n", Dxx, Dyy, Dxy);
      //   printf("response: %f\nlaplacian: %f\n\n", responses[index], laplacian[index]);
      // }
