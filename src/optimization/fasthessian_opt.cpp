...
// Only for images >= 128x128
void compute_response_layers_Dxx(struct fasthessian *fh) {
    for (int i = 0; i < fh->total_layers; ++i) {
        compute_response_layer_Dxx(fh->response_map[i], fh->iimage);
    }
}
...
void compute_response_layers_Dyy_laplacian_locality(struct fasthessian *fh) {
    for (int i = 0; i < fh->total_layers; ++i) {
        compute_response_layer_Dyy_laplacian_locality(fh->response_map[i], fh->iimage);
    }
}
...
void compute_response_layers_Dyy_laplacian_locality_unconditional(struct fasthessian* fh) {
    for (int i = 0; i < fh->total_layers; ++i) {
		compute_response_layer_Dyy_laplacian_locality_unconditional(fh->response_map[i], fh->iimage);
	}
}
...
void compute_response_layers_Dyy_Dxx_laplacian_locality_unconditional(struct fasthessian* fh) {
    for (int i = 0; i < fh->total_layers; ++i) {
		compute_response_layer_Dyy_Dxx_laplacian_locality_unconditional(fh->response_map[i], fh->iimage);
	}
}
...
void compute_response_layers_Dyy_laplacian_locality_unconditional_opt(struct fasthessian* fh) {
    for (int i = 0; i < fh->total_layers; ++i) {
		compute_response_layer_Dyy_laplacian_locality_unconditional_opt(fh->response_map[i], fh->iimage);
	}
}
...
void compute_response_layers_Dyy_laplacian_locality_unconditional_opt_flops(struct fasthessian* fh) {
    for (int i = 0; i < fh->total_layers; ++i) {
		compute_response_layer_Dyy_laplacian_locality_unconditional_opt_flops(fh->response_map[i], fh->iimage);
	}
}
...
void compute_response_layer_Dyy_top_mid(struct response_layer* layer, struct integral_image* iimage) {
    /*
        base function: compute_response_layer_Dyy_top

        optimization:
        - Dyy box filter hand optimized and split into cases
            => top, mid cases of filter response computations
    */
    float Dxx, Dyy, Dxy;
    int x, y, k, k0;
    int r00, r01, c00, c01, r10, r11, c10, c11;
...
void compute_response_layer_Dyy(struct response_layer* layer, struct integral_image* iimage) {
    /*
        base function: compute_response_layer_Dyy_top_mid
...
void compute_response_layer_Dyy_laplacian(struct response_layer* layer, struct integral_image* iimage) {
    /*
        base function: compute_response_layer_Dyy

        optimization:
        - simplified laplacian

        notes:
        - probably already done by compiler
    */
    float Dxx, Dyy, Dxy;
    int x, y, k, k0, k1;
    int r00, r01, c00, c01, r10, r11, c10, c11;
...
void compute_response_layer_Dyy_laplacian_locality(struct response_layer* layer, struct integral_image* iimage) {
    /*
        base function: compute_response_layer_Dyy_laplacian

        optimization:
        - optimized locality by going row-wise through data
    */
    float Dxx, Dyy, Dxy;
    int x, y, k, k0, k1;
    int r00, r01, c00, c01, r10, r11, c10, c11;
...
void compute_response_layer_Dxx_leftcorner(struct response_layer *layer, struct integral_image *iimage) {
    /*
        ...
        optimization:
        - Dxx box filter hand optimized and split into cases (here only leftcorner)
    */
    float Dxx, Dyy, Dxy;
    int x, y;
    int r0, r1, c0, c1, r00, r01, c00, c01, r10, r11, c10, c11;
    ...
    */
    ...
    // Top Left Corner - Case 1: C of neg part outside
...
void compute_response_layer_Dxx_top_mid(struct response_layer *layer, struct integral_image *iimage) {
    /*
        base function: compute_response_layer_Dxx_top

        optimization:
        - Dxx box filter hand optimized and split into cases (here only top and mid)
    */
    float Dxx, Dyy, Dxy;
    int x, y;
    int r0, r1, c0, c1, r00, r01, c00, c01, r10, r11, c10, c11;
    ...
    *****************/
    ...
    // Mid Left
    k = (lobe + step - 1) / step * step;
    k0 = (lobe / 2 + 1 + step - 1) / step * step;

    for (int i = k; i < height * step - lobe + 1; i += step) {
        ind = (i / step) * width;
        ...
        // Case 2: C of neg part inside
        for (int j = k0; j < border + 1; j += step) {
            // Image coordinates
            x = i;
            y = j;
    ...
    // Mid Mid
    k0 = (border + 1 + step - 1) / step * step;

    for (int i = k; i < height * step - lobe + 1; i += step) {
        ind = (i / step) * width + k0 / step;

        for (int j = k0; j < width * step - border; j += step) {
            // Image coordinates
            x = i;
            y = j;
    ...
    // Mid Right
    k0 = (width * step - border + step - 1) / step * step;
    k1 = (width * step - lobe / 2 + step - 1) / step * step;

    for (int i = k; i < height * step - lobe + 1; i += step) {
        ind = (i / step) * width + k0 / step;

        // Case 1
        for (int j = k0; j < width * step - lobe / 2; j += step) {
            // Image coordinates
            x = i;
            y = j;
        ...
        for (int j = k1; j < width * step; j += step) {
            // Image coordinates
            x = i;
            y = j;
    ...
    *****************/
    ...
    // Bottom
    k = (height * step - lobe + 1 + step - 1) / step * step;
...
// Only for images >= 128x128
void compute_response_layer_Dxx(struct response_layer *layer, struct integral_image *iimage) {
    /*
        base function: compute_response_layer_Dxx_top_mid
...
void compute_response_layer_Dyy_laplacian_locality_unconditional_opt(struct response_layer* layer, struct integral_image* iimage) {
    /*
        base function: compute_response_layer_Dyy_laplacian_locality_unconditional
...
void compute_response_layer_Dyy_laplacian_locality_unconditional_opt_flops(struct response_layer* layer, struct integral_image* iimage) {
    /*
        base function: compute_response_layer_Dyy_laplacian_locality_unconditional_opt
...
void compute_response_layer_Dyy_laplacian_locality_unconditional_opt_flops_invsqr(struct response_layer* layer, struct integral_image* iimage) {
    /*
        base function: compute_response_layer_Dyy_laplacian_locality_unconditional_opt_flops

        optimization:
        - Flops optimized even further by using scalar replacement for inverse area

        notes:
        - apparently optimizer does this better.
    */
    float Dxx, Dyy, Dxy;
    int x, y, k, k0, k1;
    int r00, r01, c00, c01, r10, r11, c10, c11;
...
void compute_response_layer_Dyy_Dxx_laplacian_locality_unconditional(struct response_layer* layer, struct integral_image* iimage) {
    ...
    */
    ...
    int x, y, k, k0, k1;
    int r00, r01, c00, c01, r10, r11, c10, c11;
    float Dyy0, Dyy1, A, B, C, D, temp0, temp1, Dxx0, Dxx1;
    ...
    *****************/
    ...
    // Inner B is outside, i.e. 0 CORRECT
    for (x = 0; x < lobe / 2 + 1; x += step) {

        // Dyy Top Left Corner Case 1 + Dxx Top Left Corner Case 1 CORRECT
        for (y = 0; y < lobe / 2 + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Left Corner Case 1 + Dxx Top Left Corner Case 2 CORRECT
        for (; y < lobe; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 1 + Dxx Top Left Corner Case 2 CORRECT
        for (; y < border + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 1 + Dxx Top Mid CORRECT
        for (; y < width * step - border; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 1 + Dxx Top Right Case 1 CORRECT
        for (; y < width * step - lobe + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Right Case 1 + Dxx Top Right Case 1 CORRECT
        for (; y < width * step - lobe / 2; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Right Case 1 + Dxx Top Right Case 2 CORRECT
        for (; y < width * step; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
    ...
    // Dyy Top Case 2 + Dxx Top Rest CORRECT
    for (; x < lobe; x += step) {

        // Dyy Top Left Case 2 + Dxx Top Left Case 1
        for (y = 0; y < lobe / 2 + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Left Case 2 + Dxx Top Left Case 2
        for (; y < lobe; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 2 + Dxx Top Left Case 2
        for (; y < border + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 2 + Dxx Top Mid
        for (; y < width * step - border; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 2 + Dxx Top Right Case 1
        for (; y < width * step - lobe + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Right Case 2 + Dxx Top Right Case 1
        for (; y < width * step - lobe / 2; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Right Case 2 + Dxx Top Right Case 2
        for (; y < width * step; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
    ...
    // Dyy Top Case 2 + Dxx Mid CORRECT
    for (; x < border + 1; x += step) {  // Inner B is inside

        // Dyy Top Left Case 2 + Dxx Mid Left Case 1 CORRECT
        for (y = 0; y < lobe / 2 + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Left Case 2 + Dxx Mid Left Case 2 CORRECT
        for (; y < lobe; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        //  Dyy Top Mid Case 2 + Dxx Mid Left Case 2 CORRECT
        for (; y < border + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 2 + Dxx Mid CORRECT
        for (; y < width * step - border; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Mid Case 2 + Dxx Mid Right Case 1 CORRECT
        for (; y < width * step - lobe + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Right Case 2 + Dxx Mid Right Case 1 CORRECT
        for (; y < width * step - lobe / 2; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Top Right Case 2 + Dxx Mid Right Case 2 CORRECT
        for (; y < width * step; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
    ...
    *****************/
    ...
    // Dyy Mid + Dxx Mid CORRECT
    for (; x < height * step - border; x += step) {

        // Dyy Mid Left + Dxx Mid Left Case 1
        for (y = 0; y < lobe / 2 + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Mid Left + Dxx Mid Left Case 2
        for (; y < lobe; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Mid Mid + Dxx Mid Left Case 2
        for (; y < border + 1; y += step) {
            // Compute Dyy  
            // whole box filter
            // All inside
        ...
        // Dyy Mid Mid + Dxx Mid Right Case 1
        for (; y < width * step - lobe + 1; y += step) {
            // Compute Dyy  
            // whole box filter
            // All inside
        ...
        // Dyy Mid Right + Dxx Mid Right Case 1
        for (; y < width * step - lobe / 2; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Mid Right + Dxx Mid Right Case 2
        for (; y < width * step; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
    ...
    *****************/
    ...
    // Dyy Bottom Case 1 + Dxx Mid Rest CORRECT
    for (; x < height * step - lobe + 1; x += step) {

        // Dyy Bottom Left Case 1 + Dxx Mid Left Case 1
        for (y = 0; y < lobe / 2 + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Left Case 1 + Dxx Mid Left Case 2
        for (; y < lobe; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 1 + Dxx Mid Left Case 2
        for (; y < border + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 1 + Dxx Mid Right Case 1
        for (; y < width * step - lobe + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Right Case 1 + Dxx Mid Right Case 1
        for (; y < width * step - lobe / 2; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Right Case 1 + Dxx Mid Right Case 2
        for (; y < width * step; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
    ...
    // Dyy Bottom Case 1 + Dxx Bottom CORRECT
    for (; x < height * step - lobe / 2; x += step) {

        // Dyy Bottom Left Case 1 + Dxx Bottom Left Case 1
        for (y = 0; y < lobe / 2 + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Left Case 1 + Dxx Bottom Left Case 2
        for (; y < lobe; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 1 + Dxx Bottom Left Case 2
        for (; y < border + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 1 + Dxx Bottom Mid
        for (; y < width * step - border; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 1 + Dxx Bottom Right Case 1
        for (; y < width * step - lobe + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Right Case 1 + Dxx Bottom Right Case 1
        for (; y < width * step - lobe / 2; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Right Case 1 + Dxx Bottom Right Case 2
        for (; y < width * step; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
    ...
    // Dyy Bottom Case 2 + Dxx Bottom Rest
    for (; x < height * step; x += step) {

        // Dyy Bottom Left Case 2 + Dxx Bottom Left Case 1
        for (y = 0; y < lobe / 2 + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Left Case 2 + Dxx Bottom Left Case 2
        for (; y < lobe; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 2 + Dxx Bottom Left Case 2
        for (; y < border + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 2 + Dxx Bottom Mid
        for (; y < width * step - border; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Mid Case 2 + Dxx Bottom Right Case 1
        for (; y < width * step - lobe + 1; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Right Case 2 + Dxx Bottom Right Case 1
        for (; y < width * step - lobe / 2; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
        ...
        // Dyy Bottom Right Case 2 + Dxx Bottom Right Case 2
        for (; y < width * step; y += step) {
            // Image coordinates
            // x = i;
            // y = j;
...
/* END OF COMPUTE RESPONSE LAYER OPTIMIZATIONS */

/* ------------------------------------------- */

/* get_interest_points optimizations */
