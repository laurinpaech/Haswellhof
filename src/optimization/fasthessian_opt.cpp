...
void super_sonic_Dyy(struct response_layer *layer, struct integral_image *iimage) {
    ...
    if (filter_size <= height) {
        ...
        // compute_response_layer_Dyy_leftcorner
    } else {
        ...
        if (height > border) {
            ...
            if (width <= lobe) {
                ...
                //  D = [height-1, width-1]
                // A, B, C = 0
                // TODO: (carla) Memset all entries to data[height-1, width-1]

            } else {
                // 2.2.b Half the filter is longer than the image, but narrower. D is outside the image, but the columns
                // change. D = [height-1, some_column] A, B = 0 C = 0 or [height-1, some_column]
            }
        }
    }
}
...
void compute_response_layer_Dyy_leftcorner(struct response_layer *layer, struct integral_image *iimage) {
    ...
    // Top Middle Part - Case 1: B of neg part outside
    for (int i = 0; i < (lobe / 2) + 1; i++) {  // Inner B is outside, i.e. 0
        for (int j = lobe; j < width - lobe; j++) {
            // Image coordinates
            x = i * step;
            y = j * step;
            ...
            C = data[r01 * width + c00];
            D = data[r01 * width + c01];
            Dyy0 = D - C;
            ...
            C = data[r11 * width + c10];
            D = data[r11 * width + c11];
