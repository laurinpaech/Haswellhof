...
void get_msurf_descriptor_gauss_compute_once_case(struct integral_image* iimage, struct interest_point* ipoint) {
    ...
    */
    ...
    int s0  = (int) roundf( 0.5 * scale);
    int s1  = (int) roundf( 1.5 * scale);
    int s2  = (int) roundf( 2.5 * scale);
    int s3  = (int) roundf( 3.5 * scale);
    int s4  = (int) roundf( 4.5 * scale);
    int s5  = (int) roundf( 5.5 * scale);
    int s6  = (int) roundf( 6.5 * scale);
    int s7  = (int) roundf( 7.5 * scale);
    int s8  = (int) roundf( 8.5 * scale);
    int s9  = (int) roundf( 9.5 * scale);
    int s10 = (int) roundf(10.5 * scale);
    int s11 = (int) roundf(11.5 * scale);

    float e_c0_m4 = (float) s2 + s1; // CAREFUL HERE!
    float e_c0_m3 = (float) s2 + s0; // CAREFUL HERE!
    float e_c0_m2 = (float) s2 - s0;
    float e_c0_m1 = (float) s2 - s1;
    //float e_c0_z0 = (float) s2 - s2;
    float e_c0_p1 = (float) s2 - s3;
    float e_c0_p2 = (float) s2 - s4;
    float e_c0_p3 = (float) s2 - s5;
    float e_c0_p4 = (float) s2 - s6;

    float e_c1_m4 = (float) s7 - s3;
    float e_c1_m3 = (float) s7 - s4;
    float e_c1_m2 = (float) s7 - s5;
    float e_c1_m1 = (float) s7 - s6;
    //float e_c1_z0 = (float) s7 - s7;
    float e_c1_p1 = (float) s7 - s8;
    float e_c1_p2 = (float) s7 - s9;
    float e_c1_p3 = (float) s7 - s10;
    float e_c1_p4 = (float) s7 - s11;
    ...
    for (int i=-8; i<8; i+=5) {
        ...
        for (int j=-8; j<8; j+=5) {
            ...
            for (int l = j-4; l < j + 5; ++l, ++gauss_index_l) {
                ...
                //Get y coords of sample point
                int sample_y = (int) roundf(ipoint_y + (l+0.5f) * scale);
                //float ys_sub_sample_y = (float) ys-sample_y;
                //float ys_sub_sample_y_squared = ys_sub_sample_y*ys_sub_sample_y;
                ...
                for (int k = i-4; k < i + 5; ++k, ++gauss_index_k) {
                    ...
                    //Get x coords of sample point
                    int sample_x = (int) roundf(ipoint_x + (k+0.5f) * scale);
                    //float xs_sub_sample_x = (float) xs-sample_x;
                    //float xs_sub_sample_x_squared = xs_sub_sample_x*xs_sub_sample_x;
